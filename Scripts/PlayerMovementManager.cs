...
public class PlayerMovementManager : MonoBehaviour
{
    ...
    bool inCrouchingProcess, dontUncrouch, slidingFromPlayerStatusManager;
    ...
    bool readyToJump = true, jumpingInput, falling, wasFalling, groundedForBouncyEnvironment, justBeforeGroundedForNormalEnvironment, justBeforeGroundedForBouncyEnvironment, playerTouchingToAnyGround;
    ...
    [SerializeField] LayerMask normalLayer, bouncyLayer;
    ...
    {// I didn't added the if not game paused condition because if game pauses, FixedUpdate pauses too.
        ...
        {// These functions' order are intentional, i wouldn't recommend you to change the order.
            ...
            Jump();
            Crouch();
            slidingFromPlayerStatusManager = PlayerStatusManager.sliding;
            LinearDamping();
    ...
    void GroundedCheckAndFallingCheckAndBouncyJumpAndCoyoteTimeAndFallDamage()
    {
        if (!crouching)
        {
            groundedForAll = Physics.CheckSphere(playerTransform.position - new Vector3(0, playerHeight / 2, 0), groundedSphereRadius, normalLayer | bouncyLayer);
    ...
    void LinearDamping()
    {
        if (groundedForAll && !jumping && !slidingFromPlayerStatusManager)
        {
    ...
    void GravityAndSpeedControl()
    {
        if (groundedForAll && playerTouchingToAnyGround && onSlope)
        {
            ...
            else if (slidingFromPlayerStatusManager)
            {
